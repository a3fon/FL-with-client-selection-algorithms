...
from ml.fl.selectors import ImportanceSelectors
from ml.fl.selectors import AccuracySelector
from ml.fl.selectors import L2Selector
...
class Server:
    def __init__(self, args, testset, model):
        ...
        #self.selector = L2Selector(args.fraction)

        #self.selector = ClusterBasedSelector(num_clusters=1)

        self.selector = ImportanceSamplingSelector(args.fraction)

        #self.selector = ImportanceSelectors(args.fraction)

        #self.selector = AccuracySelector(args.fraction)

        #self.selector = PowerOfChoiceSelector(num_clients_to_select=70, candidate_pool_size=352)
